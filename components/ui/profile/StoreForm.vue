<template>
    <Wrapper class="flex flex-col gap-y-10 font-inter">
        <TitleSection class="text-4xl">
            Store
        </TitleSection>
        <form action="" class="flex flex-col gap-y-4">
            <InputCustom label="Name" id="name" placeholder="name" v-model:model-value="storeForm.name"
                class=" lg:w-1/2"></InputCustom>
            <div class="space-y-4 lg:w-1/2 relative">
                <label for="image" class="w-full h-48 border border-dashed flex flex-col relative justify-center items-center cursor-pointer rounded-md">
                    <Icon name="heroicons:cloud-arrow-up" style="font-size: 4rem; color: gray;"/>
                ...
                <div class="w-full h-48 absolute top-0 rounded-md overflow-hidden" v-if="viewImage">
                    <NuxtImg :src="viewImage" alt="store-img" class="h-full w-full object-contain" />
            ...
            <div class="flex flex-col gap-y-2  lg:w-1/2">
                ...
                <textarea rows="8" name="desc" id="desc" v-model="storeForm.description"
                    class="border p-2 rounded-md focus:outline-0"></textarea>
            </div>
            <BaseBtn
